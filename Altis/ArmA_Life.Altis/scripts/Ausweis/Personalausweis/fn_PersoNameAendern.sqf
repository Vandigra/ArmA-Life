...
if (_Originalname isEqualTo profileName) exitWith {ERROR("Aktueller Name stimmt bereits mit deinem aktuellem Profil überein");};
...
_Aktion = [
	format["Du bist dabei den Namen der oben genannte Identität zu ändern. Dieser Vorgang kostet dich %1 € und werden von deinem Konto abgezogen.<br/>Aktuell hast du %2 € auf deinem Konto.<br/>Alter Name: %3<br/>Neuer Name: %4<br/>Möchtest du die Identität ändern?",if (_KostenIdentitaetNameAendern > 0) then {ZAHLENKONVERTER(_KostenIdentitaetNameAendern)}else{"keine"},ZAHLENKONVERTER(KONTO),_Spielername,profileName],
