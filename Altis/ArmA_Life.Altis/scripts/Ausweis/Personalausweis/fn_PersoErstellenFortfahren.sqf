...
if ((lbCursel _Listbox_TT isEqualTo -1) || (lbCursel _Listbox_MM isEqualTo -1) || (lbCursel _Listbox_JJJJ isEqualTo -1)) exitWith {hint "Keine gültigen Geburtsdaten";};
...
if (lbCursel _Listbox_Staatsangehoerigkeit isEqualTo -1) exitWith {hint "Keine Staatsangehörigkeit";};
perso_staat = (_Listbox_Staatsangehoerigkeit lbValue (lbCursel _Listbox_Staatsangehoerigkeit));
if (lbCursel _Listbox_Koerpergroesse isEqualTo -1) exitWith {hint "Keine Körpergröße";};
perso_cm = (_Listbox_Koerpergroesse lbValue (lbCursel _Listbox_Koerpergroesse));
if (lbCursel _Listbox_Koerpergewicht isEqualTo -1) exitWith {hint "Kein Körpergewicht";};
perso_kg = (_Listbox_Koerpergewicht lbValue (lbCursel _Listbox_Koerpergewicht));
if (lbCursel _Listbox_Wohnort isEqualTo -1) exitWith {hint "Kein Wohnort";};
perso_wohnort = (_Listbox_Wohnort lbData (lbCursel _Listbox_Wohnort));
if (lbCursel _Listbox_Strasse isEqualTo -1) exitWith {hint "Keine Straße";};
perso_strasse = (_Listbox_Strasse lbData (lbCursel _Listbox_Strasse));
if (lbCursel _Listbox_Hausnummer isEqualTo -1) exitWith {hint "Keine Hausnummer";};
perso_hsnr = (_Listbox_Hausnummer lbValue (lbCursel _Listbox_Hausnummer));
if (lbCursel _Listbox_Augenfarbe isEqualTo -1) exitWith {hint "Keine Augenfarbe";};
perso_augenfarbe = (_Listbox_Augenfarbe lbValue (lbCursel _Listbox_Augenfarbe));
if (lbCursel _Listbox_Geschlecht isEqualTo -1) exitWith {hint "Keine Geschlechtsauswahl";};
...
if !((_Namenskonvertierung select 1) isEqualTo "") then {
	_AnzeigeName ctrlSetToolTip format["Gelöschte Zeichen: %1\n\nIst weiter nicht schlimm. Clanzeichen oder andere Kennzeichnungen entfernt.",(_Namenskonvertierung select 1)];
	[
		"Dein Profilname enthält nicht zugelassene Zeichen, welche für den Personalausweis herausgefiltert wurden.<br/>Sollte der Name nicht vollständig oder korrekt angezeigt werden, so versuche nur einen Clantag vor oder nach dem Namen zu verwenden:<br/>- Zeichen zwischen ""["" und ""]"" werden entfernt<br/>- Zeichen zwischen ""{"" und ""}"" werden entfernt<br/>- Zeichen zwischen ""("" und "")"" werden entfernt<br/>- Beispiel: ""Max Mustermann [TNT]"" wird zu ""Max Mustermann""",
		"Namenshinweis",
		"OK",
		false
